...
export class DocumentProcessorService extends BaseService {
     ...
     */
    calculateOptimalChunkSize(document) {
     ...
     */
    async processDocumentsWithConcurrency(documents, chunkSize, overlap, concurrency) {
        const results = [];
        const errors = [];
        ...
        for (let i = 0; i < documents.length; i += concurrency) {
            ...
            const batchPromises = batch.map(async (document) => {
                try {
                    ...
                    const errorMsg = `Error processing document ${document.id || document.title}: ${error.message}`;
