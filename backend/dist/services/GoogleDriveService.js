...
export class GoogleDriveService extends BaseService {
    ...
    async initialize() {
        try {
            ...
            this.handleError(error, 'Google Drive API initialization');
        }
    }
    /**
     * List Google Docs in a specific folder or all accessible docs
     */
    async listDocuments(folderId = null, maxResults = 100) {
        try {
            if (!this.drive) {
            ...
            this.handleError(error, 'Document listing');
        }
    }
    /**
     * List ALL Google Docs from your account (not limited to folders)
     */
    async listAllGoogleDocs(options = {}) {
        try {
            if (!this.drive) {
                throw new Error('Google Drive API not initialized. Call initialize() first.');
            }
            const { maxResults = 100, includeShared = true, searchQuery = null } = options;
            ...
            this.handleError(error, 'All Google Docs listing');
        }
    }
    /**
     * Advanced search for Google Docs by content and metadata
     */
    async searchGoogleDocs(searchQuery, maxResults = 20) {
        try {
            if (!this.drive) {
                throw new Error('Google Drive API not initialized. Call initialize() first.');
            }
            ...
            this.handleError(error, 'Google Docs search');
        }
    }
    /**
    ...
    async getDocumentContent(documentId) {
        try {
            ...
            this.handleError(error, `Document content retrieval for ${documentId}`);
        }
    }
    /**
     ...
     */
    async loadDocuments(documentIds = null, folderId = null) {
        try {
            const documents = [];
            if (documentIds && Array.isArray(documentIds)) {
                ...
                for (const file of fileList) {
                    try {
                        ...
                        console.warn(`Failed to load document ${file.name}:`, error.message);
                    }
                }
            }
            ...
            this.handleError(error, 'Documents loading');
        }
    }
    /**
     * Search for documents by name
     */
    async searchDocuments(query, maxResults = 20) {
        try {
            if (!this.drive) {
                throw new Error('Google Drive API not initialized. Call initialize() first.');
            }
            ...
            this.handleError(error, 'Document search');
        }
    }
    /**
     * Load and save documents to repository
     */
    async loadAndSaveDocuments(documentIds = null, folderId = null) {
        try {
            const documents = await this.loadDocuments(documentIds, folderId);
            const savedDocuments = [];
            for (const doc of documents) {
                if (this.repository) {
                    const savedDoc = await this.repository.saveDocument({
            ...
            this.handleError(error, 'Document loading and saving');
        }
    }
}
