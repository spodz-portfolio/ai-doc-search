...
export class RagService extends BaseService {
    ...
    async query(question, options = {}) {
        try {
            const { topK = 5, minSimilarity = 0.7, includeContext = true, maxContextLength = 4000 } = options;
            // Auto-initialize RAG system if not already initialized
            if (!this.isInitialized) {
            ...
            const relevantChunks = await this.vectorStoreService.similaritySearch(question, topK, minSimilarity);
...
Please provide a comprehensive answer based on the context above.`;
            const answer = await this.chatService.generateResponse(question, [], { systemPrompt });
